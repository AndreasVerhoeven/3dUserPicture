...
protected:
	...
	bool hasLostDevice;						// indicates if we have lost the device
	...
	HRESULT NextRotation(float radRotation, float xRadRotation=0.0f, float zRotation=0.0f);
...
public:
	...
	bool FlipToBack(float rotation, float xRotation, float zRotation, Bitmap** bmp);
	bool FlipToFront(float rotation, float xRotation, float zRotation, Bitmap** bmp);
	...
	BOOL HasLostDevice()const;

	SIZE GetOptimalSize()const { return optimalSize;};

	void SetTextureBitmaps(Bitmap* front, Bitmap* back);
};
