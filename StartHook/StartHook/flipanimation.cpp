...
BOOL CFlipAnimation::HasLostDevice()const
{
	return hasLostDevice ? TRUE : FALSE;
}
...
void CFlipAnimation::SetTextureBitmaps(Bitmap* front, Bitmap* back)
{
	frontBitmap = front;
	backBitmap = back;
	...
	SIZE optSize = CalculateOptimalSizeFor(frontBitmap->GetWidth(), frontBitmap->GetHeight());
	if(optSize.cx != optimalSize.cx || optSize.cy != optimalSize.cy)
	{
		Uninitialize();
		Initialize(hwnd);
		...
		//CreateD3DPresentParameters(hwnd, optSize.cx, optSize.cy, params);
		//HRESULT hRes = directDevice->Reset(&params);
		//InitRenderTarget();
		//InitSysMemSurface();
	}
	...
	SafeRelease(frontTexture);
	SafeRelease(backTexture);
	InitTexture(&frontTexture, front);
	InitTexture(&backTexture, back);
}
...
HRESULT CFlipAnimation::NextRotation(float radRotation, float xRadRotation, float zRadRotation)
{
	HRESULT hRes = S_OK;

	D3DXMATRIX rotationMatrix ,rotationMatrix2, rotationMatrix3;
	D3DXMatrixIdentity(&rotationMatrix);
	D3DXMatrixIdentity(&rotationMatrix2);
	D3DXMatrixIdentity(&rotationMatrix3);
	D3DXMatrixRotationY(&rotationMatrix, radRotation);
	D3DXMatrixRotationX(&rotationMatrix2, xRadRotation);
	D3DXMatrixRotationZ(&rotationMatrix3, zRadRotation);

	D3DXMATRIX finalWorldMatrix = worldMatrix * rotationMatrix * rotationMatrix2  * rotationMatrix3;
...
bool CFlipAnimation::FlipToBack(float rotation, float xRotation, float zRotation, Bitmap** bmp)
{
	HRESULT hRes = S_OK;
	...
	rotationIsZero = (rotation == 0 || rotation == 180) && (xRotation == 0 || xRotation == 180) &&
						(zRotation == 0 || zRotation == 180);
	...
	hRes =  NextRotation(DegreeToRad(static_cast<float>(rotation)), DegreeToRad(xRotation), DegreeToRad(zRotation));
...
bool CFlipAnimation::FlipToFront(float rotation, float xRotation, float zRotation, Bitmap** bmp)
{
	HRESULT hRes = S_OK;
